...
function BlogList({ posts }: Props) {
  ...
  const [searchQuery, setSearchQuery] = useState(""); // State for search query    // State for visible posts
          ...
          <ClientSideRoute key={post._id} route={`/post/${post.slug.current}`}>
            ...
            transition-transform duration-200
             ease-out "
                ...
                <div className="absolute bottom-0 w-full bg-opacity-20 backdrop-blur-lg rounded drop-shadow-lg p-5 flex justify-between">
                  ...
                  <div className="flex flex-col md:flex-row gap-y-2 md:gap-x-2 items-center">
                    {post.categories?.map((category) => (
                      <p
                        key={category._id}
                        className=" text-center text-xs bg-black text-white px-3 py-1 rounded-full font-semibold"
                      >
                        {category.title}
                      </p>
                    ))}
              ...
              <div className=" mt-5">
                <p className="underline decoration-4.2 decoration-[#0A7DFF] text-lg font-bold">
                  {post.title}
                </p>
